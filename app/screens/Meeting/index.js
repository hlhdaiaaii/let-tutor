...
const Meeting = () => {
  ...
  const token = useStore(state => state.tokens.access.token);
  const userInfo = useStore(state => state.userInfo);
  const scheduleDetailInfo = data.scheduleDetailInfo;
  const startTime = moment(scheduleDetailInfo.startPeriodTimestamp);
  const endTime = moment(scheduleDetailInfo.endPeriodTimestamp);
