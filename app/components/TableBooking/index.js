...
export function TableBooking({data, tutor, page}) {
  console.log('data');
  console.log(data);
      ...
      if (existTimeSlot) {
        if (existTimeSlot.isBooked) {
          ...
          if (checkAfter2Hours(existTimeSlot.startPeriodTimestamp)) {
            return <BookBtn timeSlot={existTimeSlot} tutor={tutor} />;
          }
          return (
            <Text
...
function BookBtn({timeSlot, tutor}) {
  ...
  const onPress = () => {
    navigation.navigate(NavConfig.Screens.Booking, {timeSlot, tutor, callBack});
  };
